...
class UserActivityStateServiceTest : TestDatabase() {

    @Autowired
    lateinit var dao: UserActivityStateDao

    private lateinit var sut: UserActivityStateService

    @BeforeEach
    fun setUp() {
        sut = UserActivityStateService(dao)
    }
    ...
    fun `returns the most recently created state`() {
        dao.save(state("user-1", "OLD", Instant.now().minusSeconds(10)))
        dao.save(state("user-1", "NEW", Instant.now()))
    ...
    fun `returns only the state of the requested user`() {
        dao.save(state("user-1", "A", Instant.now()))
        dao.save(state("user-2", "B", Instant.now().plusSeconds(1)))
    ...
    fun `stores an empty string when the new state is null`() {
        ...
        assertTrue(result.isRight())
        assertEquals("", dao.findAll().single().activityState)
